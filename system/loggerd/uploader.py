...
fake_upload = True  # 默认启用假上传，禁用实际的网络上传
...
class Uploader:
  ...
  def upload(self, name: str, key: str, fn: str, network_type: int, metered: bool) -> bool:
    ...
    # 假上传模式：直接标记文件为已上传，不进行实际的网络请求
    cloudlog.info("fake_upload enabled: skipping actual upload for %s", key)
    success = True
    ...
    if success:
      ...
      try:
        ...
        cloudlog.event("uploader_setxattr_failed", exc=e, key=key, fn=fn, sz=sz)
